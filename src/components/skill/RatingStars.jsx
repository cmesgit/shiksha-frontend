 ...
 *
 * Replaces every ad-hoc "★".repeat(Math.round(r)) in the codebase. Rounding
 * to whole stars draws 4.6 and 4.9 identically, which erases the difference
 * the directory ranking is built on.
 *
 * Usage:
...
export function RatingSummary({ value, count = 0, size = 13 }) {
  const base = {
    ...
    fontSize: 12.5, color: "var(--sk-body, #5e7469)",
  };
  ...
  if (count < MIN_REVIEWS) {
    return (
      <span style={base}>
        ...
        <b style={{ color: "var(--sk-ink, #0b2e20)" }}>New</b>
  ...
  return (
    <span style={base}>
      ...
      <b style={{ color: "var(--sk-ink, #0b2e20)" }}>{Number(value).toFixed(1)}</b> ({count})
    </span>
  );
}
