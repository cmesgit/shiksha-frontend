...
const CurrentAffairs = () => {
  ...
  return (
    <div className="current-affairs-page">
      ...
      <p className="page-description">Stay updated with the latest news and current affairs from around the world</p>
      {loading ? (
        <div className="loading"><p>Loading current affairs...</p></div>
      ) : error ? (
        <div className="error">
          <p>{error}</p>
          <button onClick={fetchCurrentAffairs} className="retry-btn">Retry</button>
        </div>
      ) : (
        <div className="news-grid">
          {news.length === 0 ? (
            <div className="empty-state"><p>No articles found right now. Please check back later.</p></div>
          ) : (
