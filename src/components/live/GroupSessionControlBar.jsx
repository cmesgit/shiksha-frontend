...
export default function GroupSessionControlBar({
  ...
  admitMode = "open",
  onSetAdmitMode = null,
  // Host controls (design_handoff_live_sessions screen 07) live in a modal,
  // not the 300px right-sidebar the panel buttons below toggle — opened from
  // this "Other" menu, next to the other host-only actions already here.
  onOpenHostControls = null,
}) {
  ...
  // Derived fresh from the shared admitMode prop on every render (not local
  // state) so this stays in sync with GroupSessionClassroomUI's People-panel
  // "Require approval to join" toggle, which writes the same backend field.
  const isLocked = admitMode === "locked";
  ...
  const toggleLockSession = async () => {
    if (!isHost || !session?.id || lockBusy || !onSetAdmitMode) return;

    const nextMode = isLocked ? "open" : "locked";
    setLockBusy(true);

    try {
      await onSetAdmitMode(nextMode);
      setOtherOpen(false);
    } catch (e) {
            ...
            <div className="gs-cb-other-menu">
              ...
              {isHost && (
                <>
                  ...
                  {onOpenHostControls && (
                    <button type="button" className="gs-cb-other-item" onClick={() => { setOtherOpen(false); onOpenHostControls(); }}>
