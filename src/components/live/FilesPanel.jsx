/**
 * FILE: src/components/live/FilesPanel.jsx  (NEW — design screen 05)
 *
 * Right-hand panel, same 300px shell as the chat and people panels
 * (.gs-panel, 48px var(--gs-host) header). Everyone uploads; only the host
 * or the uploader may delete a row. Every row shows its own expiry
 * countdown, because files disappear after the admin's retention window
 * (sessions_app/live_rules.py::file_expires_at).
 *
 ...
 * receives the *same* `group_session_chat_<id>` socket GroupSessionClassroomUI
 * already holds open for chat (see that file's `chatSocket` state) and adds
 * an independent `addEventListener("message", …)` listener — additive,
 * side-by-side with the chat panel's own `ws.onmessage` handler, so neither
 * clobbers the other. `session_file_added` / `session_file_removed` match
 * the exact broadcast shape in live_files_views.py + consumers.py
...
export default function FilesPanel({ sessionId, isHost, currentUserId, limits, socket }) {
  ...
  const upload = useCallback(
    async (fileList) => {
      ...
      for (const file of incoming) {
        ...
        try {
          const { data } = await liveSessionService.uploadFile(sessionId, file, (percent) =>
            setPending((p) =>
  ...
  const remove = async (fileId) => {
    setError("");
    try {
      await liveSessionService.deleteFile(sessionId, fileId);
